...
def score(args):
    ...
    scored.to_csv("data/scored/claims_scored.csv", index=False)
    ...
    by_ep.to_csv("data/v1_outputs/results_by_episode.csv")
    ...
    lb.to_csv("data/v1_outputs/publisher_leaderboard.csv")
    ...
    if "speaker_name" in s.columns:
        ...
        if len(named):
            ...
            (named.sort_values(["hit", "confidence"], ascending=[False, True])[cols]
             .to_csv("data/v1_outputs/famous_calls.csv", index=False))
...
def livingston_directional(band_scale=1.0):
    ...
    xl = pd.ExcelFile("data/reference/medians.xlsx")
    rows = []
...
if __name__ == "__main__":
    ...
    ap.add_argument("--claims", default="data/claims/claims_graded.csv")
