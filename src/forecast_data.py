...
LIV_GROWTH = "data/reference/MedianGrowthRate.xlsx"
...
def build_multivar():
    ...
    df.to_csv("data/scored/forecasts_multivar.csv", index=False)
    print("=== MULTIVAR pool ===")
...
def build():
    ...
    df.to_csv("data/scored/forecasts.csv", index=False)
    audit(df)
    return df
