...
OUT = Path("figures/prelim_figures")
...
if __name__ == "__main__":
    sc = pd.read_csv("data/scored/claims_v2_scored.csv")
    ...
    if "episode" not in sc.columns:
        pages = {json.loads(l)["page_id"]: json.loads(l)
                 for l in open("data/corpus/pages.jsonl", encoding="utf-8")}
        sc["episode"] = sc["page_id"].map(lambda p: (pages.get(p) or {}).get("episode"))
    sc = sc[(sc["scorable"] == True) & (sc["hit"].isin([0, 1]))].copy()
    all_claims = [json.loads(l) for l in open("data/claims/claims_v2.jsonl", encoding="utf-8")]
    fig_extraction()
