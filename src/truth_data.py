...
#   STOCK  5.0%   equities swing far more than output; a real market move is bigger
DEFAULT_BANDS = {"INDPRO": 2.0, "CPI": 1.5, "UNRATE": 0.5, "STOCK": 5.0}

# Historical stock-price index (NBER "Index of Common Stock Prices for the
# United States"), monthly 1914-1968 -- covers the whole LOC newspaper period.
# `markets`-topic claims are forecasts about STOCKS, so scoring them against
# industrial production (as the first version did) is the wrong series; this is
# the right one. It doubles as the real-time asset-price signal the accuracy
# model wants as a feature.
STOCK_SERIES = "M1109BUSM293NNBR"
...
def load_fred(sid):
    ...
    Reads a local CSV if present and only hits the network otherwise. Set
    RISE_FRED_DIR to point at a folder of pre-downloaded fred_<id>.csv files --
    this is what lets the scorer run fully OFFLINE inside a walled environment
    (e.g. ProQuest TDM Studio): bundle the four tiny FRED CSVs, set the env var,
    and no request ever leaves the sandbox."""
    import os
    data_dir = Path(os.environ.get("RISE_FRED_DIR", CACHE))
    data_dir.mkdir(exist_ok=True, parents=True)
    f = data_dir / f"fred_{sid}.csv"
    if not f.exists():
        try:
            ...
            raise FileNotFoundError(
                f"{f} not found and could not download {sid} ({e}). In an "
                f"offline environment, bundle fred_{sid}.csv and point "
                f"RISE_FRED_DIR at its folder.") from e
...
class TruthData:
    ...
    def __init__(self, indpro=None, cpi=None, unrate=None, recessions=None,
        ...
        self.stocks = load_fred(STOCK_SERIES) if stocks is None else stocks
    ...
    def realized_direction(self, topic, start, horizon_months, bands=None):
        ...
        if topic == "markets":
            # Stock forecasts are scored against stock prices, not output.
            change = self._window_change_pct(self.stocks, start, horizon_months)
            if change is not None:
                return (self._band3(change, bands["STOCK"], "improve", "worsen"),
                        True, "STOCK")
            # Pre-1914 (series start): fall back to the NBER chronology, since a
            # panic reliably tanks the market too. Same fallback business uses.
            p0 = start.to_period("M")
            ...
            if not window or p1 > pd.Period("1961-12", "M"):
                return (None, False, "no stock index before 1914 / outside coverage")
            share_recession = sum(m in self.recession for m in window) / len(window)
            return ("worsen" if share_recession >= 0.5 else "improve", True, "NBER")

        if topic in ("general_business", "other"):
