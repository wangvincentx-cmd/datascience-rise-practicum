...
def stats():
    ...
    n_all = sum(1 for _ in open("claims_monthly.jsonl", encoding="utf-8"))
    return {
        "n_pages": sum(1 for _ in open("data/monthly/pages_monthly.jsonl",
                                       encoding="utf-8")),
        "n_claims": n_all,
        "n_scorable": len(s),
        "hit": s["hit"].mean(),
...
def main():
    ...
    add_text(slide, 1.3, 26.3, 8.0, 9.0, [
        ...
        ("Library of Congress Chronicling America. Every month from 1900-01 to "
         "1963-12 — 768 of 768 months, no gaps. A fixed, direction-neutral "
         "search phrase set, held constant across all months.",
         SMALL, False, INK, 12),
    ...
    y = 7.3
    add_text(slide, 11.1, y, 14.0, 1.4, [
        ("1.  The forecast mix never responded to the economy",
         34, True, INK, 0)])
    add_pic(slide, FIG / "figA_mechanism.png", 11.1, y + 1.5, 14.0)
    add_text(slide, 11.1, y + 7.0, 14.0, 1.2, [
        (f"Downbeat forecasts were {st['pess_exp']:.1%} of the total in "
         f"expansions and {st['pess_rec']:.1%} in recessions — identical. "
         f"About {st['upbeat']:.0%} of all forecasts were upbeat, in booms and "
         f"busts alike.", CAPTION, False, MUTED, 0)])

    y = 16.0
    add_text(slide, 11.1, y, 14.0, 1.4, [
        ("2.  So accuracy collapsed exactly when it mattered",
         34, True, INK, 0)])
    add_pic(slide, FIG / "figB_consequence.png", 11.1, y + 1.5, 14.0)
    add_text(slide, 11.1, y + 8.4, 14.0, 1.4, [
        (f"Overall accuracy fell from {st['hit_exp']:.1%} in expansions to "
         f"{st['hit_rec']:.1%} in recessions (gap +18.7 points, 95% CI "
         f"[+12.1, +24.8], block-bootstrapped by 3-year period). Pessimists "
         f"were right when it counted — there were never enough of them.",
         CAPTION, False, MUTED, 0)])

    y = 26.4
    add_text(slide, 11.1, y, 14.0, 1.4, [
        ("3.  Sixty years, no improvement", 34, True, INK, 0)])
    add_pic(slide, FIG / "figC_no_learning.png", 11.1, y + 1.5, 14.0)
    add_text(slide, 11.1, y + 7.3, 14.0, 1.4, [
    ...
    add_text(slide, 11.1, y, 14.0, 1.4, [
        ("4.  What predicts whether a forecast came true?",
         34, True, INK, 0)])
    add_pic(slide, FIG / "figD_what_predicts.png", 11.1, y + 1.5, 14.0)
    add_text(slide, 11.1, y + 8.8, 14.0, 2.0, [
        ("Out-of-fold AUC (leave-one-block-out, 21 three-year blocks): "
         ...
         "0.561. Both combined 0.588. We do not claim to predict which "
         "forecasts come true; 0.561 is barely above chance, and saying so is "
         "the result.", CAPTION, False, MUTED, 0)])
    ...
    add_text(slide, 26.9, 8.6, 7.9, 28.0, [
        ...
        ("On a smaller crisis-only sample (n=232), forecasts that went against "
         ...
         "finding.", SMALL, False, INK, 18),
        ("Limitations", BODY, True, INK, 6),
        ("•  The monthly corpus used a cheaper extractor (F1≈0.61) than the "
         "methods comparison (F1≈0.79), to fit a $25 budget.", SMALL, False, INK, 6),
    ...
    print(f"   {st['n_pages']:,} pages | {st['n_claims']:,} claims | "
          f"{st['n_scorable']:,} scored | hit {st['hit']:.1%}")
