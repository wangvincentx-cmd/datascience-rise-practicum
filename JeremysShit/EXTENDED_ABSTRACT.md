...
> validated at moderate confidence (see Result 6 and Limitations). This is a
...
- **Leakage discipline / evaluation.** Every feature is information-time legal;
  ...
  are.
- **Professional-forecaster benchmarks.** Three independent, publicly
  downloadable panels are scored on the identical INDPRO/NBER rule as the
  newspapers: the Survey of Professional Forecasters (individual microdata,
  1968–2026), the Philadelphia Fed's Livingston Survey (1946–2026), and the
  Federal Reserve Board staff's own real-time forecasts (the **Greenbook/
  Tealbook data set**, 490 FOMC editions, Jan 1967–Dec 2020 — released only
  after a 5-year FOMC confidentiality lag, so it cannot yet reach 2021–2026,
  but every post-1963 crisis window used here, oil_1973 through gfc_2008, sits
  well inside its coverage). A fourth, complementary data set — 8,947
  forecast-rows pooling Greenbook, Livingston, and SPF onto one
  provenance-tagged schema, joined to real-time FRED state features — powers
  the credibility model in Result 3.
...
**2. The press, the professionals, and the Fed's own staff are equally blind to
turning points — and the Fed's real-time record shows mechanically why.**
Scored on identical ground truth, four independent benchmarks converge just
above a coin flip: **Survey of Professional Forecasters 54.1%**, **Livingston
economists 54.4%**, **Michigan households ≈55%**, and **Federal Reserve Board
staff (Greenbook) 54.0%** (n=480, 95% CI [49.6, 58.3]). Crucially, **the SPF
issues a "downturn" call in essentially 0% of surveys**, and the Greenbook's
own prediction mix is nearly identical (92% improve / 7% no-change / 1%
worsen) — professionals almost never forecast a contraction a year out (cf.
Loungani's "failure to predict recessions"). The Greenbook's own internal
record explains *why*, not just *that*: across 490 editions, forecast
dispersion collapses from a 4.01-point SD at the nowcast to 0.90 points at
8 quarters out while the mean holds near +2.8%, and the share of below-zero
calls falls from 12.9% to **exactly 0.0%** — beyond about a year, the
Greenbook simply *is* a trend forecast, and a constant cannot call a turning
point by construction. Consistent with this, in the twelve months before
every one of the 8 NBER peaks from 1969–2020 the mean one-year-ahead
Greenbook forecast was positive; only 3 of 8 (1969, 1980, 1981) ever produced
even a single negative call, and the four most recent — 1990, 2001, 2007
(the GFC; mean +2.08%, worst call still +1.45%), and 2020 (COVID, mean
+2.08%) — were missed entirely. Skill relative to a naive "always improve"
baseline is **~0 in every decade from the 1970s through the 2010s** (n=480);
the 1990s' apparently strong 85% raw hit rate is purely that decade's
recession-free naive baseline itself hitting 83.8%, not forecasting skill —
the same trap this project already applies to its own newspaper numbers. This
sharpens the arm's central claim: the newspapers' optimism bias is not
amateurism — the best-resourced, most information-rich forecaster in the
country shows the identical structural bias, and by construction cannot call
a turn more than about a year out. A Diebold–Mariano test finds **no
...
**3. Forecast credibility is real, general, and modest — and it tracks
turning-point proximity, not the forecaster.** A complementary question to
"was this specific claim right": given a forecast and the economy's state
when it was made, can we predict whether it will come true? Pooling
Greenbook, Livingston, and SPF individual microdata onto one
provenance-tagged schema (8,947 rows, 1946–2026) and fitting a logistic
credibility model per source, 5-fold expanding-window cross-validation gives
ROC-AUC **0.74 [0.44–0.99] (Greenbook)**, **0.66 [0.55–0.85] (SPF)**, **0.70
[0.50–0.89] (Livingston, small n)** — real but modest, with intervals wide
for the same reason Result 1's episode-level test is underpowered: only
~8–12 US recessions to learn from, regardless of row count. Cross-source
transfer is high both directions (Livingston→Greenbook 0.84 AUC,
SPF→Greenbook 0.83) — credibility is a general property of forecasts, not
one source's idiosyncrasy. An ablation shows why the sources differ: for
Greenbook, both the forecast's own features and the economic state matter
(0.79 forecast-only / 0.77 state-only / 0.81 combined), but for SPF the
forecaster's own specific call adds almost nothing (0.55, chance) while the
state alone carries it (0.76) — an individual professional's number carries
little beyond knowing where the business cycle already stands. Dropping the
NBER-final features that wouldn't have been knowable in real time
(`usrec_now`, `expansion_age`) for a deployable, real-time-safe feature set
(adding a yield-curve spread) costs only ~0.03 AUC, confirming the result is
not a look-ahead-leakage artifact. Read together with Results 1 and 2: every
forecaster type here is reliable in stable expansions and unreliable exactly
at the turn that matters — this is a calibration finding, not a
recession-prediction model.

**4. A calibration correction.** Replacing the LLM's low-reliability confidence
...
**5. A claim's direction interacts with time and policy uncertainty — a lead,
...
**6. Narrative Economics — a moderate-confidence reversal of the naive
...
- **Residual label noise** (~3% of `is_prediction=yes` on spot-check) and one
  ...
  κ-validated at moderate confidence (κ=0.58, n=80) — see Result 6.
...
- **The Greenbook/credibility results share the same underlying power limit,
  in a different guise.** Greenbook, Livingston, and SPF all forecast the
  same handful of US recessions (~8–12 since 1946), so the credibility
  model's 8,947 pooled rows are not 8,947 independent episodes — the wide
  cross-validated AUC intervals in Result 3 (e.g. Greenbook [0.44, 0.99]) are
  that same constraint made visible on a different model. The 5-year FOMC
  confidentiality lag also means Greenbook cannot yet reach 2021–2026, and
  the credibility model's economic-state features use revised FRED series
  rather than real-time (RTDSM) vintages — the deployable, real-time-safe
  ablation in Result 3 shows this costs only ~0.03 AUC, so it is disclosed as
  a refinement, not treated as a live hole. Both sources were restricted to
  the US by design (Fed SEP and all international series were excluded to
  keep the comparison apples-to-apples), which caps turning-point power
  further but keeps the benchmark honest.
...
only 19 episodes. The Federal Reserve's own real-time record shows this is
not a press or amateur failing: the Greenbook's forecast dispersion
mechanically collapses to a trend line beyond about a year out, missing every
one of the four most recent NBER recessions, and a complementary credibility
model finds the same story from another angle — a forecast's trustworthiness
transfers across forecaster type but tracks proximity to a turn, not the
forecaster's own identity or stated number. The contribution is as much
methodological as substantive: a validated LLM-grading pipeline with strict
information-time discipline makes a century of qualitative forecasts
quantitatively scorable, and disciplined testing (permutation, Diebold–
Mariano, calibrated bands, cross-source transfer, and — new — testing at the
episode rather than claim level) keeps the honest negatives honest.
...
- `figures/fig_greenbook_benchmark.png` — Greenbook 54.0% joins the convergence, prediction-mix comparison
- `figures/fig_greenbook_anatomy.png` — forecast SD collapse by horizon, missed-recession calls, skill-by-decade
- `figures/fig_credibility_calibration.png` — credibility model calibration curve, per source
- `figures/fig_credibility_coefficients.png` — forecast-own vs economic-state feature importance (the ablation)
- `figures/fig_credibility_transfer.png` — cross-source AUC transfer matrix (Greenbook/Livingston/SPF)
